...
int main() {
    ...
    json_t* int_json = json_number(42);
    assert(int_json != nullptr);
    assert(json_typeof(int_json) == JSON_NUMBER);
    assert(json_is_number(int_json) == 1);
    assert(json_number_value(int_json) == 42);
    
    // Test negative integer
    json_t* neg_int_json = json_number(-123);
    assert(neg_int_json != nullptr);
    assert(json_number_value(neg_int_json) == -123);
    
    // Test zero
    json_t* zero_json = json_number(0);
    assert(zero_json != nullptr);
    assert(json_number_value(zero_json) == 0);
